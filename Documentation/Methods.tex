...
(commonly $10\,\mathrm{K/min}$) under controlled atmosphere, and records
...
\begin{itemize}
  \item \emph{Gas buoyancy.} The sample pan displaces a volume of the
        ...
        $10^{-3}\,\%$ per $\degC$ over a typical ramp. The
...
worse it gets: noise of $0.001\,\%$ in $W$ at $0.05\,\degC$ spacing
becomes noise of about $0.02\,\%/\degC$ in $\dwdT$, comparable to the
...
$0.3\,\degC$ and $5\,\degC$, compute $\rho_1$ for each, and pick the
...
In practice on $10\,\mathrm{K/min}$ runs of $\mathrm{CaCO}_3$ powders
the procedure picks windows of $0.6$--$1.2\,\degC$; for noisier
hydrated-cement-paste data it can grow to $4$--$5\,\degC$. The chosen
...
\begin{itemize}
  \item a \emph{prominence} threshold (default $2\,\%$ of the run's
        maximum $|\dwdT|$) to filter out noise peaks, and
  \item a \emph{minimum separation} (default $20\,\degC$) so a single
        decomposition with internal wiggles is not double-counted.
\end{itemize}
For each surviving peak we walk outward in $T$ until $|\dwdT|$ drops
below a small fraction of the peak amplitude --- typically $2\,\%$. We
...
$44\,\%$ of its mass has different buoyancy and geometry than the
...
sides. In a hydrated cement paste below $200\,\degC$, the
...
\begin{itemize}
  \item $A$ is the peak amplitude (in $\%/\degC$).
...
extent, so a fit on the carbonate region around $700\,\degC$ cannot
be contaminated by the residual exponential tail of a peak the fit
also placed at, say, $200\,\degC$. Each peak influences only the
...
keep $A \ge 0$, $W \ge 1\,\degC$, and $|b| \le 0.95$ to avoid
degenerate fits.
...
\subsection{The region must include quiet pads}

By far the most common deconvolution mistake is choosing a region that
starts or ends inside a tail of a real event. The package fits the
pre-baseline to a $20\,\degC$-wide window just below $T_{\mathrm{lo}}$
and the post-baseline to a $20\,\degC$-wide window just above
$T_{\mathrm{hi}}$. If either of those windows is contaminated by
ongoing chemistry, the baseline slope is wrong, the corrected DTG is
wrong, and the deconvolved per-peak masses are nonsense --- even
though the fit converged.

A worked example. On a clean calcite (theoretical mass loss
$43.97\,\%$), a deconvolution with $T_{\mathrm{lo}} = 700\,\degC$,
which sits well inside the rising edge of the decarbonation event,
gives per-peak mass losses of about $21$, $27$, and $27\,\%$ under the
three baseline modes. Widening to $T_{\mathrm{lo}} = 540\,\degC$ ---
which puts the pre-pad in a genuinely quiet region --- gives $43.6$,
$43.7$, $43.7\,\%$: a $0.3\,\%$ spread, agreeing with theory. The fit
converged in both cases. The only difference is whether the pre-pad
saw chemistry or not.
...
\begin{itemize}
  \item \emph{Multi-rate (isoconversional) experiments.} Ramp the same
        sample at, e.g., $5$, $10$, $20$, and $40\,\mathrm{K/min}$,
...
\begin{itemize}
  \item Vaterite $\to$ calcite, roughly $320$--$460\,\degC$.
  \item Aragonite $\to$ calcite, roughly $380$--$490\,\degC$.
  \item $\mathrm{CaCO}_3 \to \mathrm{CaO} + \mathrm{CO}_2$ (calcite),
        roughly $600$--$800\,\degC$, depending on crystallinity,
...
``aragonite that became calcite at $430\,\degC$.'' Coupled techniques
...
from a hydrate carbonate, but the $700\,\degC$ decarbonation peak by
...
\begin{enumerate}
  ...
  \item In that case, run a deconvolution over a region wide enough
        to include the event \emph{and} $20\,\degC$ of quiet baseline
        pad on each side, with $N$ peaks chosen to match what the DTG
        plot suggests.
