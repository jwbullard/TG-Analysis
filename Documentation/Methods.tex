...
sluggish, sometimes piled on top of a neighbor.
...
\begin{itemize}
  ...
  \item and, when neighboring events overlap, a defensible way to
...
$2k+1$ nearest neighbors by ordinary least squares, then evaluates
that polynomial at the center point to get the smoothed value. The
...
positions of the window's points relative to its center, $i$. On a
...
correlation, so $\rho_1$ depends only on whether neighboring
...
the way to a neighboring peak without ever finding a quiet point ---
...
explicitly modeled because each baseline is only evaluated at its own
end.}
\label{fig:tangent}
\end{figure}
...
the integration extent sweeps up neighboring chemistry, and the
...
\begin{itemize}
  \item \emph{Multi-rate (isoconversional) experiments.} Ramp the same
        ...
        and analyze the apparent activation energy as a function of
